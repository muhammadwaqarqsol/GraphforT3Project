...
import { Buy, Token, User, Transfer } from "../generated/schema";
import { BigInt, Value, ethereum, ipfs, json } from "@graphprotocol/graph-ts";
...
export function handleNFTBought(event: NFTBoughtEvent): void {
  ...
  let Buyer = new Buy(event.transaction.hash.concatI32(event.logIndex.toI32()));
  ...
  Buyer.price = event.params.price;
